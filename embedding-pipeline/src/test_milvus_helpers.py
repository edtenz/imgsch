...
def test_insert():
    ...
    milvus_cli.create_collection(table_name, dim)
    ...
    vec_id = milvus_cli.insert(table_name, obj_feat.features)
    print('insert vec id:', vec_id)
    res = milvus_cli.search_vectors(table_name, obj_feat.features, top_k)
...
def test_search_vectors():
    ...
    print('candidate_box:', candidate_box)
    print('vector size:', len(obj_feat.features))
    res = milvus_cli.search_vectors(DEFAULT_TABLE, obj_feat.features, 10)
    print(res)


def test_delete_collection():
    res = milvus_cli.delete_collection(DEFAULT_TABLE)
    print(res)
...
def test_search_milvus_ops():
    ...
    p_search_pre = (
        VitBase224().pipeline()
        .flat_map('vec', ('pred', 'distance'), search_milvus_ops(milvus_cli, table_name, 5))
